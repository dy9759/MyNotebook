...
interface AddToNotebookDialogProps {
  ...
  /** Single source ID or array for batch mode */
  sourceId?: string
  sourceIds?: string[]
  sourceTitle?: string
}
...
export function AddToNotebookDialog({
  ...
  const addToNotebook = useAddSourcesToNotebook()
  ...
  const handleAdd = async () => {
    if (selectedNotebooks.size === 0 || ids.length === 0) return

    for (const notebookId of selectedNotebooks) {
      await addToNotebook.mutateAsync({
        notebookId,
        sourceIds: ids,
      })
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <DialogFooter>
          ...
          <Button
            onClick={handleAdd}
            disabled={selectedNotebooks.size === 0 || addToNotebook.isPending}
          >
            {addToNotebook.isPending ? (
